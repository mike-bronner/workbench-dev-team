...
description: Local scheduled task. Polls Calvinball MCP for work in each of the three agent lanes every 20-30 minutes, then fires the appropriate subagent (Miss Wormwood, Tracer Bullet, Moe) as a detached subprocess per item.
---
...
### Lane 1 — Wormwood (triage)

```
items = mcp__calvinball__list_unrefined_items()
for each item in items:
  dispatch Wormwood on item.id
```

Dispatch command (run in Bash, **detached**):

```bash
nohup claude -p --agent workbench-dev-team:miss-wormwood \
  --model haiku \
  --dangerously-skip-permissions \
  --no-session-persistence \
  "<ITEM_ID>" \
  > "$HOME/.claude-workbench/dev-team-logs/wormwood-<ITEM_ID>-$(date +%Y%m%d-%H%M%S).log" 2>&1 &
disown
```

### Lane 2 — Tracer Bullet (review)

```
items = mcp__calvinball__list_review_items()
for each item in items:
  dispatch Tracer on item.id
```

Dispatch command:

```bash
nohup claude -p --agent workbench-dev-team:tracer-bullet \
  --model sonnet \
  --dangerously-skip-permissions \
  --no-session-persistence \
  "<ITEM_ID>" \
  > "$HOME/.claude-workbench/dev-team-logs/tracer-<ITEM_ID>-$(date +%Y%m%d-%H%M%S).log" 2>&1 &
disown
```

### Lane 3 — Moe (development)

```
items = mcp__calvinball__list_development_items(limit=1)
if items is non-empty:
  dispatch Moe on items[0].id
```

Dispatch command (note the budget cap):

```bash
nohup claude -p --agent workbench-dev-team:moe \
  --model opus \
  --dangerously-skip-permissions \
  --no-session-persistence \
  --max-budget-usd 5.00 \
  "<ITEM_ID>" \
  > "$HOME/.claude-workbench/dev-team-logs/moe-<ITEM_ID>-$(date +%Y%m%d-%H%M%S).log" 2>&1 &
disown
```

Moe is single-track: the server returns at most one item, and Moe's own `/tmp/moe.lock` prevents a second Moe from stomping on a currently-running one.

## Rules

- **Fire-and-forget.** Every dispatch goes into the background with `nohup ... &` + `disown`. Never wait for an agent to complete — Moe alone can run for hours.
...
- **Final output.** Print a one-line-per-dispatch summary:
  `→ wormwood #123 (repo/name)`
  `→ tracer #456 (repo/name)`
  `→ moe #789 (repo/name)`
