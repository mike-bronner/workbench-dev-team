...
- **Interactive (`orchestrate`)** passes the config's `model` as the Agent tool's per-invocation model override. The Agent tool has no per-invocation effort, budget, or fallback parameter, so `effort` reaches this path through the agent definition instead: setup stamps each agent's configured value into its frontmatter (and removes the line for an agent the config gives none, as it does for Watson), and the harness reads it there when it spawns the sub-agent. `maxBudgetUsd`/`fallback` have no equivalent route and stay scheduled-path only (a model error there surfaces immediately for you to handle).
...
The agent definitions carry matching frontmatter defaults (`model: sonnet|opus`, `effort: high` for Holmes and Lestrade), so a direct Agent-tool dispatch without the skill still lands on the right model *and* the right effort. Setup's Step 6a rewrites that `effort` line from this config on every run — and removes it when the config names no effort, so the frontmatter stays silent in exactly the case where Dispatch omits `--effort`.

**Watson ships no `effort` at all**, on either path: no key in the config, no line in its frontmatter. That is deliberate, and it is why the block above is asymmetric. Watson shipped `xhigh` for months before anyone read the line, and shipping a corrected number would only have left the next wrong number just as easy to miss — a value that is never shipped cannot drift. So a Watson dispatched interactively inherits the calling session's effort, and a scheduled one takes the model's own default. Holmes and Lestrade keep `high` because both run rarely enough that the same drift risk is small beside a review- or triage-quality regression. `agents/test-effort-stamp.sh` holds the config and the frontmatter to each other in both directions — including on Watson's absence, where re-adding the key to one and not the other turns the suite red.
