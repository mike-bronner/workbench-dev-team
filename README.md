...
Also defines the **commit approval gate** (see [Commit approval gate](#commit-approval-gate) below): a sub-agent commits, merges, and pushes nothing, no foreground `git commit` or `git push` the gate can read runs without your explicit approval of that exact command, and a push that forces or deletes remote refs is refused outright.
...
**A sub-agent does not commit, does not merge, and does not push. In the foreground session, the agent attempts every `git commit` and `git push` itself, as a plain one-line command, and each one waits for your explicit approval.** Non-negotiable, and enforced by the harness rather than by prose — prose alone drifts, and this gate has the measurements to prove it.
...
| **Foreground session** | an empty `agent_id` | A plain one-line `git commit` or `git push` once you have approved that exact command. Any other command that could hide one is refused and asked for in the plain form. A push that forces or deletes remote refs is refused outright. Merge, rebase, pull, revert, cherry-pick, am, and `gh pr merge` stay your own, and the gate has no opinion on them |
...
**Why a foreground push is gated too.** The gate used to wave a foreground push through, and the prose called push "the human's own". Agents read that as "hand the push to you", so they stopped short of it and asked you to push. A push that did run went out with no prompt at all. A `permissions.ask` rule for `git push` cannot fix this, because an unattended `claude -p` pipeline run cannot answer one (workbench-core's `rails.json` records this under HEADLESS CONSTRAINT). The hook can carry the pipeline exemption, so the push prompt lives here, on the commit's own route. The agent attempts the push, and the gate asks you. The approval script needed no push variant: a request record is keyed by the exact command text, so the same script approves either one. A push carries no subject, so its approval command takes none, and the receipt names the push and what it is bound to: `✅ Approved: git push origin feature (main at 1a2b3c4d5e6f)`.

**A push that forces or deletes is refused, not approved.** workbench-core's rails deny `git push --force` by prefix and nothing else, so an approval spent on that spelling would only buy a second denial, and every other spelling would run. You chose to refuse deletion pushes the same way, rather than name them in a prompt. The gate refuses these with no request id and no record, and a planted approval buys nothing, the way it refuses a sub-agent:

| Refused as | Spellings the gate reads |
|---|---|
| force | `--force`, `--force-with-lease[=…]`, `--mirror`, any prefix git accepts (`--force-w`, `--mirr`), `-f` in any short cluster, a refspec with a leading `+` |
| deletion | `--delete`, `--prune`, their prefixes (`--de`, `--pru`), `-d` in any short cluster, a refspec with an empty source (`:branch`, `:refs/heads/x`) |
| either, from config | `remote.<r>.mirror`, or a `remote.<r>.push` refspec that does either, in the repository's own config. On the command line, `-c` is not the plain form, so it is refused as class (b) |

Quoting does not hide any of them: in the plain form the gate removes quotes exactly as the shell does, and nothing else is there to expand. A bare `:` refspec pushes matching branches and is not a deletion, and `-o` takes the rest of its word, so `-oF` is a push option. Both stay on the approval path. A force or deletion push stays yours to run.

**The gate does not parse bash or zsh.** Two review rounds taught this. Splitting on whitespace missed subshells, wrappers, and quoting. A shlex tokenizer then let the apostrophes in two `#` comments pair up and hide a commit and a push between them, silently, in both lanes. Each parser was a new set of shell it got wrong, and the Bash tool runs zsh here, which is a second grammar. So the gate recognises a few plain shapes, and sorts every command into one of three classes:

| Class | What it is | What happens |
|---|---|---|
| (a) plain form | One line, exactly `git [-C <path>] commit …`, `git [-C <path>] push …`, or `git [-C <path>] commit … && git [-C <path>] push …` | Prompted, with the force and deletion refusal and the bindings below |
| (b) could hide one | Anything else whose raw text names git or yadm and a verb word, or names git or yadm and holds a character the shell builds words from, or names `gh` and a verb word | Refused with no request id: "Run it as a plain line of its own." |
| (c) everything else | Including a plain one-line git command whose verb is not gated, such as `git status` or `git log --grep push` | Untouched |

**The plain form's alphabet.** Every word is bare or quoted. A bare word holds ASCII letters, digits, and `- _ . / : = + @ , % ^`, and may not start with `=`, because zsh expands `=ls` to `/bin/ls`. A single-quoted string holds anything but `'`. A double-quoted string holds anything but `$`, a backtick, or a backslash, because bash or zsh expands those inside it. The program `git` and the verb are bare. Nothing else is allowed: no expansion, substitution, glob, brace, redirect, pipe, comment, heredoc, assignment, wrapper such as `env` or `time`, `cd`, second line, or separator other than the one `&&`. So the gate knows exactly which words git receives. Put a multi-line message in a file and commit with `-F <absolute path>`, run `git add` as its own call, and use `-C <path>` rather than `cd`. Claude Code's default `git commit -m "$(cat <<'EOF' … EOF)"` is refused, and that cost was accepted.

**The class (b) test is substring-level, with no parsing.** It runs case-insensitively over the raw text, so no quote, comment, or line break can move a word out of view. The verb words are `commit`, `push`, `send-pack` (the plumbing under push, refused outright rather than prompted), `autocorrect` (which turns a typo into either), and `alias` (which renames one); a sub-agent adds `merge`, `rebase`, `pull`, `revert`, `cherry`, and `am` as a word. The shell-building characters are `$ ` \ ' " { } * ? [ ]`. `git` counts when no letter follows it, so `github` is not git, and `gh` counts only as a word of its own.

**The cost is deliberate.** Some innocent commands are refused: `git log --oneline | grep commit`, `echo "git commit"`, or `cd repo && git log --format='%h'`. A refusal costs one retry, on its own line; a false silence is an unapproved push. A plain one-line call to a command that cannot run another command from its arguments (`log`, `show`, `diff`, `status`, `stash`, `help`, and the rest of `SAFE_VERBS`) stays silent even when its text names a verb. Several of those write, such as `add`, `reset`, and `tag`; "safe" means only that no argument makes them commit or push. That keeps the common cases free. `git rebase --exec 'git push'` and `git submodule foreach 'git push'` are refused.

**What stays outside, stated plainly.** This is not anti-evasion machinery. A command built to hide from a substring test can: a program name spelled in pieces the text never shows whole, a script file, or a gitconfig alias or `help.autocorrect` setting reached through a command that names neither verb.

**Shell aliases and functions are the live case.** The Bash tool runs zsh with your shell snapshot, and that snapshot defines oh-my-zsh's git aliases and `g=git`: `gp`, `gcmsg`, `gcam`, `gwip`, `gpf!`, `gpod`, `g push`. Their text names neither git nor a verb, so each one is silent in both lanes, exactly as it was before this gate existed. The gate reads the command text, and the shell expands the alias after it. Every shape Holmes listed in three review rounds is class (a) or class (b), and each one is a test.

**How a foreground approval happens.** The denial is not a dead end, it is an instruction:

1. The gate denies the commit or push. You read ``🛑 Blocked: `git commit`. It needs your approval first.`` (or `` `git push` ``); the agent reads the request id and the instructions in `additionalContext`.
2. The agent shows you the diff and the proposed message for a commit, or the branch, remote, and commits a push sends (the prose layer, now with teeth). For a push, the context also names the branch and commit the approval is bound to.
3. The agent runs `bash "$HOME/.claude-workbench/bin/approve-commit.sh" <id> "<subject>"`, or the same command with no subject for a push. Two `permissions.ask` rules cover that command, so the harness raises a **real** prompt: permission rules are evaluated before the classifier in every mode.
4. You answer the prompt. That answer is the approval. `approve-commit.sh` confirms it back in one line that names everything approved: `✅ Approved: <subject>` for a commit, `✅ Approved: <push statement> (<branch> at <commit>)` for a push, and `✅ Approved: <subject>, then <push statement>` for both. A commit's command is never reprinted, because you have already seen it twice by then and it can carry a whole message. A push statement is short, so it is how a push is named. The agent re-runs the same command from the same directory, and the gate steps aside once.

`bin/approve-commit.sh` is installed at that stable path by `/workbench-dev-team:setup`, which also adds the rules. **It refuses to approve anything while the rules are missing, or when run from any copy but the installed one** — a half-finished setup blocks commits instead of waving them through. `Bash(git commit:*)` and `Bash(git push:*)` are deliberately *not* ask rules: an ask rule always prompts, and the headless pipeline has nobody to answer it (workbench-core's rails exclude it for the same reason).

**A message kept in a file is approved on the same terms.** `git commit -F <path>` is the shape to prefer, because the gate makes the command run twice and a message written inline is printed twice in full in your transcript. It is also the only way to a multi-line message, since the plain form is one line. The subject you read still has to be the real one, so `approve-commit.sh` reads the first line of the file git will read — `-F <path>`, `--file=<path>`, `--file <path>`, and a cluster like `-aF`. The path is the literal text git receives, because the plain form holds no variable, and it must be absolute. Every check that decides whether an approval may be granted finishes before a byte of that file is read, and the file supplies one thing: the subject. So a file can refuse an approval or name a commit, and never widen one. A relative path, a file that will not open, a file with no message in it, and `-F -` (standard input, gone by the time an approval runs) each refuse and name their own case. One consequence is worth stating plainly: the approval covers the command, so a file rewritten between the prompt and the commit changes the message without changing what was approved.

**One approval covers one run of one command, and nothing else.** Each request id is a hash of the session id, the agent id, the working directory, and the exact command text. Two sessions never share an approval, editing the command voids it, running it from another directory voids it, and no sub-agent can spend one — it is refused before the record is even read. The record is deleted the moment the command uses it, and it expires after 15 minutes unused.

**An approval is also bound to what the command writes.** The gate reads it when it records the request, and again when the approved command runs, and refuses on any difference with a fresh id:

| Command | Bound to | Refusal on a change |
|---|---|---|
| commit | HEAD and the staged diff (`git diff --no-ext-diff --no-textconv --cached --binary`, so no configured diff program can stand in for the content), plus the working-tree diff when the commit takes files from there: `-a`, `--all`, `-i`, `--include`, `-o`, `--only`, `-p`, `--patch`, `--interactive`, a bare `-U`, or a pathspec | ``🛑 Blocked: `git commit`. The staged changes differ from what you approved.`` |
| push | the repository (at the `-C` path as git resolves it, symlinks included, or the working directory), HEAD, the branch HEAD names, every local branch and tag tip, and every `remote.*`, `branch.*`, `push.*`, and `url.*` setting | ``🛑 Blocked: `git push`. The repository changed since you approved it.`` |

So a loop cannot spend one commit approval twice on new content, and an ungated pull, rebase, cherry-pick, or config change before a push voids the push approval. In `commit && push`, both are read before the command runs: the push then sends the approved state plus the one commit whose staged content you approved. A command whose repository the gate cannot read is refused rather than approved blind.
...
**What this gate is not.** It is not anti-evasion machinery. It reads the command an agent asked to run, within the limits listed under "The gate does not parse bash or zsh", and in the foreground lane the approval record is a file that anything holding Bash can write. An invocation of `approve-commit.sh` spelled differently from the two rules (`sh <path>`, or the path without the `bash` prefix) is not matched by them and so is not prompted, which is why the denial prints the exact command to run. A hook payload that will not parse still yields no opinion, as it always has. What it does guarantee: a sub-agent's commit, merge, or push is refused with nothing to run in reply, a plain-form foreground commit or push cannot happen *silently*, any other command that names one is refused, and every failure of the gate's own machinery — missing rules, unwritable state, no session id — refuses the call rather than permitting it.

Tests: `hooks/scripts/test-commit-approval-gate.sh` (309 cases — detection, non-commit silence, the two-channel split asserted field by field (the human line's exact text, its one-line length, and the absence of the id, the command, the policy and any Markdown emphasis from it; the context dictating the approval prompt's `description` and still carrying the id), the full approval lifecycle, the session/command binding, the sub-agent lane in full (every refused verb, no id and no approval command in *either* channel, no record written under its own key, and a hand-planted approval buying it nothing), two cases pinning `agent_id` rather than `agent_type` as the lane signal, the foreground lane leaving merge, rebase, pull, and `gh pr merge` alone, a foreground push denied until approved and then let through once (the human line, the unlabelled approval command, the push `description`, and a commit-and-push named as both), every force and deletion spelling in the table above refused with no id, no record, and nothing a planted approval can buy, the non-force neighbours (`:`, `-oF`, `--dry-run`) still prompted, every shape from both review rounds as class (a) prompted or class (b) refused, the class (b) cases asserted in both lanes — the comment-apostrophe script and its lines, fake heredoc openers, expansions in the program or verb slot, `git-push`, `help.autocorrect`, loops, `pull && push`, subshells, groups, wrappers, paths, case, quoting, redirects, aliases, and the stated false-refusal costs — the class (c) innocent commands silent in both lanes, the approval bound to its directory, a push bound to its repository's branches, tags, HEAD, and remote, branch, push, and url config, a commit bound to HEAD and the staged diff and, for `-a`, `--all`, `-i`, `--only`, `-p`, `--patch`, `--interactive`, a bare `-U`, and a pathspec, the working tree, with an external diff driver unable to hide a change and no textconv driver run, `-C lnk/..` bound to the repository git resolves through the symlink, a combined `commit && push` voided by a staged change and by a ref change, `send-pack` and a zsh `=word` refused, the fail-closed paths each asserted by their own human line, the carve-out's exact-match semantics including a flagged agent run, a guard proving no path can return `ask` again, and a two-part regression guard proving a live `watson.lock` no longer bypasses). `bin/test-approve-commit.sh` (120 cases — every refusal asserted twice, by exit status and by the commit still being denied afterwards, the end-to-end deny → approve → commit-once path, the same path for a push with the receipt naming the push, its branch, and its commit, a label refused on a push and when it comes from the push statement, push options that look like message flags (`-ocheck=confirm`, `-oF`) read as push options, a commit-and-push receipt naming both, a two-push, env-prefixed, or variable-path command issued no id, an approval voided by a branch that moved, a record that does not say what it approves refused, the receipt naming the commit without reprinting the command on both the labelled and unlabelled paths plus its request-id fallback, a sub-agent that is issued no id and cannot spend the foreground's, and the message-file path in full: each spelling git accepts, each unreadable, relative, empty and standard-input case refusing by its own named message, a label checked against the file's first line rather than the command text so a path fragment cannot pass as a subject, and the approval staying bound to the command when the file is rewritten under it). `commands/test-commit-approval-setup.sh` (19 cases — the setup block is extracted from the Markdown and run for real against a sandbox `HOME`).
