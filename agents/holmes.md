...
#### 4e. Defects and observations beyond the AC — route by coupling, then locality
...
These come from the surviving (UPHELD, deduped) findings of the correctness / security / test-honesty lenses in Phase C — or, in the fallback, from your own inline read. Beyond the AC contract (§4d), two axes decide each finding's fate: **how serious** it is (a hard defect vs. a softer observation) and **where** it lives (`in-pr` — on a line this PR added or modified — vs. `general` — code the PR left untouched). But locality is only the *starting* axis: **coupling beats locality.** A finding in untouched code that *this PR's change made stale, inconsistent, or wrong* is the PR's mess to clean up — it blocks, exactly as if it were in-diff — because the diff is what broke it. So the untouched-code column splits by causation:

| | In the PR's diff (`in-pr`) | Untouched code the diff broke (`general`, **caused by this PR**) | Untouched code, independent of this PR (`general`, **pre-existing**) |
|---|---|---|---|
| **Hard defect** — correctness, security, or test | 🔴 **blocker** | 🔴 **blocker** | 🔴 **blocker** |
| **Soft observation** — refactor, duplication, minor improvement | 🔴 **blocker** | 🔴 **blocker** | 🟡 **non-blocking follow-up** |
...
- **🔴 Untouched code that this PR's change made stale, inconsistent, or wrong blocks — even a soft one.** Locality answers *"did the diff touch this line?"*; that misses *causation* — a change can leave untouched code stale, contradictory, or wrong *without* touching it, and that staleness is part of the change, not a separate cleanup. The crisp self-test:
  > **"Would this finding still be true if this PR had never happened?"** — If **no** (the diff is what introduced it), it is in-scope for *this* PR: block it and fix it here. If **yes** (it's a pre-existing condition, independent of this diff), it is the genuine non-blocking follow-up tier.

  When the answer is *no*, the finding blocks regardless of severity — a now-stale rationale aside, a comment the change falsified, a doc the change contradicts. Keep this test **tight**: the bar is *causation by this diff*, not loose "relatedness." Untouched code that is merely *near* the change, or that you'd *like* cleaned up while someone's in there, is not made wrong by the diff — don't block it. Only what the diff actually broke crosses into the PR.
- **🟡 Only a soft observation about untouched code that is independent of this change is non-blocking.** That — and only that — is the follow-up tier: a soft observation that is both (a) not named by any AC item **and** (b) not made stale or wrong by this diff (the self-test above answers *yes* — it would still be true if the PR had never happened). It's still a real, actionable thing a person could pick up and *do* — "extract this duplicated parser into a helper (`x.ts:40`, `y.ts:55`)" — but it lives outside the PR's changes, no AC item names it, and the diff didn't cause it. Collect it as a `note` and carry it into the **`## 📋 Non-blocking follow-ups`** section of your verdict (§5), where it gets dispositioned **by verdict path** rather than dying in your context: **on approve it becomes a tracked issue** (you route it — there's no Watson round-trip to catch it), and **on request-changes Watson implements it in the same bounce PR** alongside the blocker fixes (he's already in the code, so a separate issue would just be churn). Hold the bar high: something doable, not "consider renaming this someday." Vague observations are noise; leave them out.
...
- **Route findings by coupling, then locality and severity.** Anything actionable in the code this PR wrote or changed is a **blocker** — request changes, *however minor* (no severity floor; convention-conformant style isn't a finding at all). A **hard defect** (correctness / security / test) blocks wherever it lives, even in untouched code. And **untouched code that this diff made stale, inconsistent, or wrong blocks too — coupling beats locality** — run the self-test *"would this finding still be true if this PR had never happened?"*: **no** (the diff caused it) → in-scope, block and fix it here; **yes** (pre-existing, independent of the diff) → follow-up tier. Only a **soft observation about untouched code that no AC item names *and* the diff didn't cause** is non-blocking — that's the follow-up tier. Keep the test tight: causation by this diff, not loose "relatedness."
