...
**Drop the "only Status field_changes" audit predicate entirely.** Its original
purpose was to avoid re-triaging an item whose status had been reset to null
after AC were already written. With a dedicated `Inbox` lane that no longer
applies: items enter `Inbox`, Wormwood triages them and moves them *out* to
`Backlog`, and nothing moves them back to `Inbox`. The lane itself is the
"untriaged" signal — no audit inspection needed.

- Replace the `->unrefined()` query scope with `->inbox()` (or rename), keyed on
  the Status value `Inbox`.
- Update the **MCP tool description string** for `list_unrefined_items` — it
  currently says "status is null". It must say it returns items in `Inbox`
  awaiting triage. The agents and Dispatch read these descriptions; drift here is
  a silent breakage.
- No data migration needed (0 null-status rows exist), but confirm.
- Tests: a feature test asserting `Inbox` items are returned and every other
  status is excluded.
...
**The PR-exclusion needs precise semantics.** The board contains both
issue-backed items and PR-backed items (e.g. Dependabot PRs auto-added to the
project). Confirm in your code which of these two the current "no PRs" filter
actually does, and land on the correct one:

1. ✅ **Keep:** exclude project items whose underlying content is a **PR**
   (`issue_number` null / `pr_number` set). Moe develops issues, not PR items.
2. ❌ **Remove (if present):** excluding an *issue* merely because a PR is
   associated with it. Moe opens a **draft PR on the first commit** of every
   fresh run (the "visible WIP" pattern), so an in-flight `In Progress` issue
   *always* has a draft PR. Excluding on that basis is precisely what breaks
   resume. An `In Progress` issue with an open draft PR authored by Moe
   (branch `moe/<issue>-*`) **must** be returned so Moe can resume it.

Since lifecycle is already encoded in Status (a merged-PR issue would be
`Approved`/`Done`, never `Ready`/`In Progress`), the simplest correct rule is:
filter out PR-type *items*, and do **not** filter issues on PR association at
all. Verify against your current implementation and reconcile.

- Add/adjust a query scope, e.g. `->readyOrInProgress()` ordered In-Progress-first.
- Tests: feature tests for (a) In-Progress returned before Ready, (b) an
  In-Progress issue with a draft PR is still returned, (c) PR-type items are
  excluded, (d) blocked items excluded, (e) `limit=1` honored.
...
- **`project_fields`** — the field catalog. For every single-select used in
  triage (`Status`, `Size`, `Business Value`, `Risk Reduction`, `Time Sensitive`,
  `Estimate`, `Priority` — use the board's actual field names), include the
  field's id/name and its full ordered list of options as `{ id, name }`. Where
  an option's `name` is a numeric label (the WSJF Fibonacci values), that's what
  Wormwood counts and selects, and what `update_fields` maps a numeric back to.
  This is the load-bearing addition.
- **`field_changes`** — the field-change audit trail for the item. Now that
  Task 1 drops the audit predicate this is contract-completeness rather than
